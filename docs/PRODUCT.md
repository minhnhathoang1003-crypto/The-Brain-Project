...
## Sửa trong 0.5.2 — ba luồng người dùng đang hỏng

**Đổi credit từ lớp phủ không bao giờ chạy.** `verify()` chỉ nhận sự kiện đến từ cửa sổ chính, mà lớp phủ là một `BrowserWindow` khác, nên mọi lần bấm nút mở đều trả về "Không được phép". Nay lớp phủ được cấp đúng một quyền: `redeem` cho **đúng mục đang bị che**, chỉ khi lớp phủ đang hiện. Mọi thứ khác — đổi cài đặt, mở website khác, lấy mã ghép nối — vẫn bị từ chối, và có kiểm thử cho từng trường hợp.

**Hủy hộp chọn ứng dụng làm hỏng mọi hộp xác nhận sau đó.** Hộp chọn ẩn nút *Xác nhận* đi và chỉ khôi phục ở nhánh chọn thành công; hủy giữa chừng là nút biến mất vĩnh viễn, khiến không dừng được phiên, không xóa được website, không bật được khóa. Nay `ask()` luôn khôi phục nút. Thêm một số hiệu hộp thoại để danh sách ứng dụng trả về muộn không ghi đè lên hộp xác nhận đang mở.
